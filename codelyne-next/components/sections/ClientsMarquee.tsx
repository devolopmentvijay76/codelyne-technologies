...
function LogoCard({ client, colorIdx }: { client: ClientItem; colorIdx: number }) {
  ...
  return (
    ...
    >
      ...
      <span className="text-sm font-semibold whitespace-nowrap" style={{ color: `${color}dd` }}>
        {client.name}
      </span>
    </div>
  );
...
export function ClientsMarquee() {
  ...
  const displayClients: ClientItem[] = (dbClients.length > 0 ? dbClients : FALLBACK_CLIENTS) as ClientItem[];
  const minItems = 10;
