...
import {
  LayoutDashboard, LogOut, Users, Edit, Trash2, Plus, Save,
  Image as ImageIcon, Upload, ArrowUp, ArrowDown,
  Package, MessageSquare, Calendar, Mail, Building2, Phone,
} from "lucide-react";
...
export default function AdminPage() {
  ...
  const form = useForm<z.infer<typeof employeeSchema>>({
    ...
    defaultValues: {
      name: "", role: "", department: "Engineering", memberType: "engineer",
      photoUrl: "", description: "", quote: "", focusAreas: "",
    },
  });
  ...
  const productForm = useForm<z.infer<typeof productSchema>>({
    ...
    defaultValues: {
      ...
      icon: "Cpu", status: "active", displayOrder: 0,
    },
  });
  ...
  const handleSaveVisionMission = () => {
    ...
    if (visionContent) {
      updateContent({ key: "vision", value: vision }, { onSuccess: () => toast({ title: "Vision Updated" }) });
    } else {
      createContent({ key: "vision", value: vision }, { onSuccess: () => toast({ title: "Vision Created" }) });
    }

    if (missionContent) {
      updateContent({ key: "mission", value: mission }, { onSuccess: () => toast({ title: "Mission Updated" }) });
    } else {
      createContent({ key: "mission", value: mission }, { onSuccess: () => toast({ title: "Mission Created" }) });
    }
  ...
  const handleDeleteEmployee = (id: number) => {
    deleteEmployee(id, {
      onSuccess: () => toast({ title: "Team Member Deleted", variant: "destructive" }),
  ...
  const handleAddProduct = () => {
    ...
    productForm.reset({
      name: "", tagline: "", description: "", features: "",
      logoUrl: "", icon: "Cpu", videoUrl: "", usp: "", domains: "",
      status: "active", displayOrder: sortedProducts.length + 1,
    });
  ...
  const handleEditProduct = (product: any) => {
    ...
    productForm.reset({
      ...
      logoUrl: product.logoUrl || "", icon: product.icon || "Cpu",
      videoUrl: product.videoUrl || "", usp: product.usp || "",
      domains: product.domains || "",
      status: product.status, displayOrder: product.displayOrder || 0,
    });
    setIsProductDialogOpen(true);
  };
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <Card className="bg-white/5 border-white/10">
                <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-gray-400">Total Team</CardTitle></CardHeader>
                <CardContent><div className="text-3xl font-bold text-white" data-testid="text-employee-count">{employeesLoading ? "..." : employees.length}</div></CardContent>
              </Card>
              <Card className="bg-white/5 border-white/10">
                <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-gray-400">Founders</CardTitle></CardHeader>
                <CardContent><div className="text-3xl font-bold text-primary" data-testid="text-founders-count">{employeesLoading ? "..." : founders.length}</div></CardContent>
              </Card>
              <Card className="bg-white/5 border-white/10">
                <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-gray-400">Engineers</CardTitle></CardHeader>
                <CardContent><div className="text-3xl font-bold text-blue-400">{employeesLoading ? "..." : employees.filter(e => e.memberType === "engineer").length}</div></CardContent>
              </Card>
              <Card className="bg-white/5 border-white/10">
                <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-gray-400">Management</CardTitle></CardHeader>
                <CardContent><div className="text-3xl font-bold text-purple-400">{employeesLoading ? "..." : employees.filter(e => e.memberType === "management").length}</div></CardContent>
              </Card>
            </div>
            ...
            <Card className="bg-white/5 border-white/10">
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <Button onClick={handleFixPhotos} disabled={isFixingPhotos} className="bg-primary text-background font-bold" data-testid="button-fix-photos">
            ...
            <Card className="bg-white/5 border-white/10">
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                {!isEditingVision ? (
                  <Button variant="outline" size="sm" onClick={() => setIsEditingVision(true)} className="border-primary/50 text-primary hover:bg-primary/10" data-testid="button-edit-content">
                      ...
                      <div key={member.id} className="bg-white/5 border border-white/10 rounded-xl p-4 hover:border-primary/30 transition-all group" data-testid={`card-founder-${member.id}`}>
                        <div className="flex items-start gap-4">
                          <div className="flex flex-col gap-1">
                            <Button variant="ghost" size="icon" className="h-6 w-6 text-gray-500 hover:text-primary disabled:opacity-30"
                          ...
                          <tr key={member.id} className="hover:bg-white/5 transition-colors group" data-testid={`row-employee-${member.id}`}>
                            <td className="p-4">
                              <div className="flex items-center gap-1">
                                <Button variant="ghost" size="icon" className="h-6 w-6 text-gray-500 hover:text-primary disabled:opacity-30" onClick={() => handleMoveUp(teamMembers, index)} disabled={index === 0 || isEmployeeReordering} data-testid={`button-move-up-${member.id}`}>
                                  <ArrowUp className="w-3 h-3" />
                                </Button>
                                <span className="text-xs text-gray-500 min-w-[16px] text-center">{index + 1}</span>
                                <Button variant="ghost" size="icon" className="h-6 w-6 text-gray-500 hover:text-primary disabled:opacity-30" onClick={() => handleMoveDown(teamMembers, index)} disabled={index === teamMembers.length - 1 || isEmployeeReordering} data-testid={`button-move-down-${member.id}`}>
                                  <ArrowDown className="w-3 h-3" />
                                </Button>
                              </div>
                            </td>
                    ...
                    <form onSubmit={productForm.handleSubmit(onSubmitProduct)} className="space-y-4">
                      ...
                      <FormField control={productForm.control} name="name" render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl><Input {...field} placeholder="CogniFlow ERP" className="bg-white/5 border-white/10 text-white" data-testid="input-product-name" /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                      <FormField control={productForm.control} name="tagline" render={({ field }) => (
                        <FormItem>
                          <FormLabel>Tagline</FormLabel>
                          <FormControl><Input {...field} placeholder="AI-powered enterprise resource planning" className="bg-white/5 border-white/10 text-white" data-testid="input-product-tagline" /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                      <FormField control={productForm.control} name="description" render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl><Textarea {...field} placeholder="Full description of the product..." className="bg-white/5 border-white/10 text-white min-h-[100px]" data-testid="input-product-description" /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                      <FormField control={productForm.control} name="features" render={({ field }) => (
                        <FormItem>
                          <FormLabel>Features (comma-separated)</FormLabel>
                          <FormControl><Input {...field} placeholder="Real-time analytics, AI predictions, Automated workflows" className="bg-white/5 border-white/10 text-white" data-testid="input-product-features" /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                      <FormField control={productForm.control} name="videoUrl" render={({ field }) => (
                        <FormItem>
                          <FormLabel>Video / Share Link</FormLabel>
                          <FormControl><Input {...field} placeholder="https://youtube.com/watch?v=... or any video link" className="bg-white/5 border-white/10 text-white" data-testid="input-product-video-url" /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                      <FormField control={productForm.control} name="usp" render={({ field }) => (
                        <FormItem>
                          <FormLabel>Unique Selling Points (USP)</FormLabel>
                          <FormControl><Input {...field} placeholder="10x faster|AI-powered|Zero downtime|Enterprise-grade" className="bg-white/5 border-white/10 text-white" data-testid="input-product-usp" /></FormControl>
                          <p className="text-xs text-gray-500">Separate each USP with a pipe character |</p>
                          <FormMessage />
                        </FormItem>
                      )} />
                      <FormField control={productForm.control} name="domains" render={({ field }) => (
                        <FormItem>
                          <FormLabel>Industry Domains</FormLabel>
                          <FormControl><Input {...field} placeholder="FinTech,HealthTech,Manufacturing,Retail,Logistics" className="bg-white/5 border-white/10 text-white" data-testid="input-product-domains" /></FormControl>
                          <p className="text-xs text-gray-500">Separate domains with commas</p>
                          <FormMessage />
                        </FormItem>
                      )} />
                      <div className="grid grid-cols-2 gap-4">
                        <FormField control={productForm.control} name="icon" render={({ field }) => (
                          <FormItem>
                            <FormLabel>Icon Name</FormLabel>
                            <Select onValueChange={field.onChange} value={field.value}>
                              <FormControl>
                                <SelectTrigger className="bg-white/5 border-white/10 text-white"><SelectValue placeholder="Select icon" /></SelectTrigger>
                              </FormControl>
                    ...
                    {sortedProducts.map((product, index) => (
                      <div key={product.id} className="p-4 bg-white/5 rounded-lg border border-white/10 flex items-center gap-3 group hover:border-primary/30 transition-colors" data-testid={`card-product-${product.id}`}>
                        <div className="flex flex-col gap-1 shrink-0">
                          <Button variant="ghost" size="icon" className="h-6 w-6 text-gray-500 hover:text-primary disabled:opacity-20" disabled={index === 0 || isProductReordering} onClick={() => handleMoveProductUp(index)} data-testid={`button-product-up-${product.id}`}>
                            <ArrowUp className="w-3 h-3" />
                          </Button>
                          <Button variant="ghost" size="icon" className="h-6 w-6 text-gray-500 hover:text-primary disabled:opacity-20" disabled={index === sortedProducts.length - 1 || isProductReordering} onClick={() => handleMoveProductDown(index)} data-testid={`button-product-down-${product.id}`}>
                            <ArrowDown className="w-3 h-3" />
                          </Button>
                        </div>
          ...
          <div className="space-y-6">
                ...
                <div className="space-y-5 py-2">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <label className="cursor-pointer">
                      ...
                      <input type="file" accept="image/*" className="hidden" disabled={isUploadingClientLogo} onChange={handleClientLogoUpload} data-testid="input-client-logo" />
                    </label>
                    {clientLogoPreview && (
                      <button className="text-xs text-red-400 hover:text-red-300" onClick={() => { setClientLogoPath(""); setClientLogoPreview(""); }}>
                        Remove logo
                      </button>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input value={clientName} onChange={e => setClientName(e.target.value)} placeholder="e.g. Acme Corporation" className="bg-white/5 border-white/10 text-white" data-testid="input-client-name" />
                  </div>
                </div>

                <DialogFooter>
                  <Button variant="ghost" onClick={() => setIsClientDialogOpen(false)} className="text-gray-400">Cancel</Button>
                  <Button onClick={handleSaveClient} className="bg-primary text-background font-bold" disabled={!clientName.trim() || isUploadingClientLogo} data-testid="button-save-client">
            ...
            <Card className="bg-white/5 border-white/10">
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <Button onClick={() => handleOpenClientDialog()} className="bg-primary/10 text-primary border border-primary/30 hover:bg-primary hover:text-background" data-testid="button-add-client">
              ...
              <CardContent>
                {clientsLoading ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {clients.map(client => (
                      <div key={client.id} className="flex items-center gap-4 p-4 bg-white/5 rounded-xl border border-white/10 group hover:border-primary/30 transition-colors" data-testid={`card-client-${client.id}`}>
                        ...
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                          <Button variant="ghost" size="icon" className="h-8 w-8 text-gray-400 hover:text-white" onClick={() => handleOpenClientDialog(client)} data-testid={`button-edit-client-${client.id}`}>
                            <Edit className="w-4 h-4" />
                          </Button>
                          <Button variant="ghost" size="icon" className="h-8 w-8 text-red-400 hover:text-red-300 hover:bg-red-500/10"
                            onClick={() => deleteClient(client.id, { onSuccess: () => toast({ title: "Client Removed", variant: "destructive" }) })}
        ...
        {activeTab === "enquiries" && (
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card className="bg-white/5 border-white/10">
                <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-gray-400">Total Enquiries</CardTitle></CardHeader>
                <CardContent><div className="text-3xl font-bold text-white">{submissionsLoading ? "..." : submissions.filter(s => s.type === "enquiry").length}</div></CardContent>
              </Card>
              <Card className="bg-white/5 border-white/10">
                <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-gray-400">Demo Requests</CardTitle></CardHeader>
                <CardContent><div className="text-3xl font-bold text-primary">{submissionsLoading ? "..." : submissions.filter(s => s.type === "demo").length}</div></CardContent>
              </Card>
            </div>
                      ...
                      <tbody className="divide-y divide-white/5">
                        {submissions.filter(s => s.type === "demo").map((submission) => (
                          <tr key={submission.id} className="hover:bg-white/5 transition-colors group" data-testid={`row-demo-${submission.id}`}>
                            ...
                            <td className="p-4 text-right">
                              <Button variant="ghost" size="icon" className="h-8 w-8 text-red-400 hover:text-red-300 hover:bg-red-500/10 opacity-0 group-hover:opacity-100 transition-opacity"
                                onClick={() => deleteSubmission(submission.id, { onSuccess: () => toast({ title: "Deleted", variant: "destructive" }) })}
                                disabled={isDeleting} data-testid={`button-delete-demo-${submission.id}`}>
                                <Trash2 className="w-4 h-4" />
                              </Button>
                            </td>
                          </tr>
                        ...
                        {submissions.filter(s => s.type === "enquiry").map((submission) => (
                          <tr key={submission.id} className="hover:bg-white/5 transition-colors group" data-testid={`row-enquiry-${submission.id}`}>
                            ...
                            <td className="p-4 text-right">
                              <Button variant="ghost" size="icon" className="h-8 w-8 text-red-400 hover:text-red-300 hover:bg-red-500/10 opacity-0 group-hover:opacity-100 transition-opacity"
                                onClick={() => deleteSubmission(submission.id, { onSuccess: () => toast({ title: "Deleted", variant: "destructive" }) })}
                                disabled={isDeleting} data-testid={`button-delete-enquiry-${submission.id}`}>
                                <Trash2 className="w-4 h-4" />
                              </Button>
                            </td>
                          </tr>
